...
- The right rail scrolls **Status → Project Map → TODO**. The Project Map slot sits between Status and TODO; each card is event-driven and only repaints when its own state changes, so a model switch or cost tick refreshes the header, not the rail. Every card (sidebar or not) paints the same rose frame — the rounded border in the theme's plain border role, the title in the accent role — the look every `CARD_TONE.INFO` card in Gentle Shell uses (warning/error/success cards keep their own tone colors).
...
`alt+m` folds both groups when either is expanded, or unfolds both once all are folded. `GENTLE_PI_PROJECT_MAP_KEY` rebinds it; an empty value keeps `alt+m`, and `off` disables the shortcut. `alt+j` and `alt+k` select the next and previous capability without wrapping, expanding a collapsed capabilities group when the selection moves onto a row it would hide; `GENTLE_PI_PROJECT_MAP_NEXT_KEY` and `GENTLE_PI_PROJECT_MAP_PREV_KEY` rebind them with the same empty-value and `off` semantics. Clicking a capability selects it, clicking it again clears the selection, and the selected row uses `▸ ` instead of its two-space indent while retaining its lifecycle glyph.

Selection appends an Inspector after Coverage with the capability's state, outcome, surfaces, foundations, dependencies, contracts, feature documents, and explicit `none` values for empty lists. Its blockers are static only: the blocked state and non-done referenced foundations or dependencies. The runtime overlay is not wired into the card yet, even though the coordination store and the protocol that reads it now exist, so the card states that rather than fabricating runtime claims, leases, or blockers. When a selected row falls outside the fullscreen rail viewport, the rail reveals it; an already visible selection does not move the viewport. The collapse key appears in the card top rule where it fits. A ready artifact appears by default; use `/gentle:project-map show` to show an empty or invalid map for the current session, and `/gentle:project-map hide` to remove it again. Visibility, collapse, and selection choices are in memory only and are discarded on session shutdown. In narrow or regular mode the same card remains visible below the editor as a non-interactive one-line summary — the `foundations · capabilities` completion line for a ready map, and the first line of the card body for an empty or invalid artifact (the missing-artifact notice, or `The Project Map artifact is not valid:` with its details still reachable through `/gentle:project-map status`) — so a notice never disappears together with the rail. The artifact itself is created and completed from the same command: `/gentle:project-map draft`, then `declare <capability-id> <surface>...` for every capability, then `approve <actor>`; the [Project Map reference](project-map.md) documents that loop, the frozen vocabulary, and the refusals.

The Inspector ends with an `[Open Pi]` line for a capability the runtime launch gate permits, and that line is absent rather than disabled when anything disqualifies it: an unapproved or non-`ready` capability, unready dependencies, an open blocker, a proposed contract without a decision, no live claim, a claim held by another live session that is neither this session nor the live lead, an occupied target, an unavailable `tmux`, or a worktree that has not been provisioned. The line is informational — the launch itself is `/gentle:project-map open <capability-id>`, which prints the plan, asks once, and starts one session in the capability worktree. When `tmux` is unusable the confirmation offers a background subagent instead, and declining leaves everything untouched. The command reports the launch as requested, never as work begun, and then reports separately whether the child proved it started by writing its own binding and heartbeat; a launch that never does is reported as `stayed unconfirmed`.
