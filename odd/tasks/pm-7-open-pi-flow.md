...
- [x] **PM7-2 — The launch plan and the tmux adapter**
  - Design, fixed before the source write:
...
- [x] **PM7-3 — The visible fallback to a background subagent**
  - Design, fixed before the source write:
    ...
    - **The subagent path reuses the existing runner**: the launch goes through `lib/agents-runner.ts`'s existing spawn contract (`pi --mode rpc`, `childArguments`), with the same handoff text and the capability worktree as `cwd`. No second runner is written. **Corrected in delivery, with measured evidence:** what is reused is the argument construction, not `AgentRunner`; the mode pair is replaced by `--print` because an undriven `--mode rpc` child reads EOF and exits without running a turn, and the fallback is a detached one-shot launch with no runner supervision.
...
- 2026-09-26: **PM7-2 delivered** (`fc13aaad`, 207 diff lines: the plan/execute split, the array `argv` with no shell, the pinned handoff shape, the named detached tmux session with collision refusal and printed attach command, and the deterministic launcher resolution with `project-map-open-pi/launcher-unavailable`) and (`d53d34ab`, 257 diff lines: the `open` sub-action with a single confirmation, the `session_start` receiver that writes the child's own binding and heartbeat without stealing the claim, and the memoized rail probe with body and digest sharing one readiness decision). Closure evidence: focused 20, neighbours 52, full suite 3,819 (3,781 passed, 38 skipped, 0 failed), type gate 195 with no regressions, provider contract, runtime harness and `git diff --check` all exiting 0. The parent added the missing no-UI path test and its honest message after a mutation survived the writer's suite.
- 2026-09-26: **PM7-3 delivered** in three commits. `f14a9dc4` (384 diff lines) is the slice: `planProjectMapOpenPiFallback` (offered only when the sole unmet readiness gate is the host; `project-map-open-pi/session-dir-required`; transport-only refusals do not refuse it), `runProjectMapOpenPiFallback` (refusing plans never spawn; `mkdir` then a detached spawn whose `spawn`/`error` settlement is awaited before any report), the explicit `select` choice with the exact tmux and subagent labels, the host-unavailable confirmation, the no-UI refusal, and two distinct non-confirmatory success notifications. `79f72ea8` (15 insertions, 6 deletions) is the prerequisite gate: `create` means the worktree is not provisioned, so readiness reports `project-map-open-pi/worktree-not-provisioned` and the fixtures provision a real worktree. `a5cb84c4` (38 insertions, 9 deletions) reports a rejected confirmation dialog instead of letting the rejection escape the command. The slice cost 452 changed lines against a 250-line forecast: the estimate held for the writer's first pass and the rest is the two audit rounds, which is the cost this plan already budgeted for a slice that introduces a human gate.
  - **The mechanism was corrected against measured evidence, not against the frozen text.** The plan named `pi --mode rpc` with `childArguments`, but an rpc child is driven: `pi --mode rpc --no-tools "<prompt>" < /dev/null` exits 0 with no response (a positional prompt is ignored and EOF ends it), `pi --mode rpc --print` is still rpc, and only `pi --print "<prompt>"` answers. The plan therefore replaces the `["--mode","rpc"]` pair with `--print` and appends the handoff as the one-shot message; every other argument still comes from `childArguments`. A spawn ACK is not a launch: the settlement wait exists because an asynchronous `error` otherwise escaped as an uncaught exception after a "launch requested" report.
  - **Two verification rounds, both read-only.** The first found the asynchronous spawn failure, the offer title that always blamed the host, the inherited `GENTLE_SHELL_INTERACTIVE_HOST` signal that `agents-runner` strips from every child, and (pre-existing, Medium) the `create` worktree reaching launch; the second confirmed those closures and found the rejected-dialog escape plus a weakened path assertion. All are closed.
  - **Closure evidence**: parent mutation matrix of fifteen distinct mutations with **zero survivors**, every probe restored byte-for-byte (sha256 identical); focused 26, neighbours 121, full suite **3,829 (3,791 passed, 38 skipped, 0 failed)**, type gate 195 with no regressions, provider contract, runtime harness and `git diff --check` all exiting 0.
  - **Recorded debt, deliberately not closed**: the fallback reuses `childArguments`/`piCommand` but is not `AgentRunner`. It installs no child markers, no IPC channel and no lifecycle supervision, because a runner-owned marker without a runner would lie; it is a detached one-shot launch contract, and it says so. Model execution, parent communication and eventual completion of the fallback child remain **unproven**.
  - **Deliberately not verified**: no real `pi` or `gentle-shell` has been launched as a child of either path, so `confirmed` remains unproven for both; the tmux path's confirmation rests on the handler-level receiver tests, and the fallback's on the pinned argv plus the detached spawn shape. The tmux integration tests do run against the real binary when `tmux` is present (it is, 3.6, so they ran rather than skipped).

## Next decision

PM7-4 is the next work unit: the two honest launch states, the acceptance trace, the documentation and the explicit record of what is not verified. It also still owns the readiness coverage limits recorded below under PM7-2 — no stale-claim fixture, no lead-held-claim allowance fixture, no separate nested-repository / foreign-clone / path-escape cases, and no test of the returned `claim` field when the lead holds the claim. PM7-3 added the `unprovisioned worktree` and `other claim` cases to the disqualifier table but did not close those four.
