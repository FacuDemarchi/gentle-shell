...
- **Semantics settled during the fix rounds** (three independent verification rounds found four real defects; all are closed and regression-tested, so do not re-derive them):
  - `directory.insideAnotherRepository` means "the target resolves to a repository that is not this clone" (`commonDir` comparison), **not** a comparison of the resolved top level against the target path — that comparison made a foreign repository rooted exactly at the target look safe, and would also have made PM6-2 refuse to reuse its own linked worktrees.
