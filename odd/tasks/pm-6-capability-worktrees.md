...
- **Review disposition (user's decision, 2026-09-25)**: RDD is enabled globally, but the user explicitly leaves **every PM-6 candidate unreviewed until the unit closes**. The review pass then reviews each part as its own candidate from a worktree pinned at that part's tip, with `baseRef` equal to the previous part's tip — so PM6-1's candidate is `bca15ce3..9fa3ae02`, not an accumulated range against the v3.7.0 base. During implementation an unreviewed-candidate reminder is answered by reporting this disposition; no review is started, and never against a tree with a writer still running.
