...
- [ ] **PM4-1 — Store schema and canonical root**
  ...
  - Design, fixed before the source write (2026-09-24):
    ...
    - One generic record API instead of five functions per record type: `validateProjectMapStoreValue(kind, value)`, `parseProjectMapStoreValue(kind, text)`, `readProjectMapStoreValueFile(kind, path)`, `canonicalizeProjectMapStoreValue(kind, value)` and `serializeProjectMapStoreValue(kind, value)`, each returning the mirrored `{ value | record, diagnostics }` result and never throwing. Five functions beat thirty-five, and the kind discriminant keeps the types honest. `schema` inside every document must equal the frozen version, and a document whose declared kind does not match the requested kind is refused rather than coerced.
    ...
    - Record shapes, all canonical with fixed key order: descriptor `{ schema, repository_id, generation, epoch, predecessor, created_at, updated_at }` where `generation` is a non-negative integer, `epoch` is a UUID and `predecessor` is a `sha256:` digest or null; claim `{ schema, capability_id, session_id, acquired_at, expires_at, lease }`; heartbeat `{ schema, session_id, pid, incarnation, beat_at }`; session binding `{ schema, session_id, pid, incarnation, workspace_root, bound_at }`; blocker `{ schema, capability_id, reason, raised_by, raised_at, resolved_at?, resolution? }`; readiness receipt `{ schema, capability_id, issued_at, verified, evidence, authority: "none" }`.
    - **Design refinement, flagged for the user**: the lease is a sub-record of the claim (`acquired_at`, `expires_at`, plus a `lease` object carrying the renewal window) rather than a separate lease document. A standalone lease file would duplicate the claim and give the store two sources of truth for the same fact, which is exactly the drift the store exists to prevent. Lease *semantics* (renew, expire, stale recovery) are PM4-3's work and are unchanged by this choice.
