...
- [ ] **PM4-1 — Store schema and canonical root**
  ...
  - Design, fixed before the source write (2026-09-24):
    ...
    - Record shapes, all canonical with fixed key order: descriptor `{ schema, repository_id, generation, epoch, predecessor, created_at, updated_at }` where `generation` is a non-negative integer, `epoch` is a UUID and `predecessor` is a `sha256:` digest or null; claim `{ schema, capability_id, session_id, acquired_at, lease }`; heartbeat `{ schema, session_id, pid, incarnation, beat_at }`; session binding `{ schema, session_id, pid, incarnation, workspace_root, bound_at }`; blocker `{ schema, capability_id, reason, raised_by, raised_at, resolved_at?, resolution? }`; readiness receipt `{ schema, capability_id, issued_at, verified, evidence, authority: "none" }`.
