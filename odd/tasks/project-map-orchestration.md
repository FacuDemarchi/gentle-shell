...
- [x] **PM-4 — Build the shared cross-worktree coordination store**
  ...
  - The last three slices were implemented with RDD switched off by the user's decision, so they carry parent verification instead of a native review; a deferred review pass over those three parts closed when the unit closed.
...
- [x] **PM-5 — Establish lead/satellite coordination contracts**
  ...
  - The whole unit was implemented with RDD switched off by the user's decision, so it carries parent verification plus independent audits of PM5-1, PM5-2 and PM5-3 instead of native reviews; the deferred review pass over the five parts closed on 2026-09-25 with 5 of 5 candidates approved and their authority burned.

- [x] **PM-6 — Provision and manage capability worktrees safely**
  - Planned 2026-09-25 in `odd/tasks/pm-6-capability-worktrees.md`, with the user's decisions: a capability worktree lives at `<parent>/<repo>-worktrees/<capability-id>` with branch `feat/<capability-id>`; PM-6 creates it behind an explicit confirmation after showing the plan; a live claim on the capability is required first; and an existing same-clone worktree on the capability branch is reused — a dirty but otherwise correct one is reported as dirty in the confirmed plan so the human decides — while a conflicting target is refused or asked about. The unit is five slices, each its own commit, and the read-only surface map recorded that nothing in product code creates, moves or deletes a capability worktree or branch today — this is the first unit that writes outside the coordination store.
  - **PM6-1 delivered** 2026-09-25 (`a4b65a07`, with the nested-versus-foreign fix `9fa3ae02`): capability worktree identity and a read-only safety inspection, 500 lines in one commit by the user's decision, 25% over the review budget with the overage recorded. **PM6-2 delivered** 2026-09-25 as two commits (`b6c77fe0` 187 lines, `0057899f` 415 lines, the second 15 lines over the budget with the overage recorded): the read-only plan with its six named refusals, and the locked provisioning behind an approved plan. Both slices were gated green before their commits and PM6-2b is byte-identical to the independently verified candidate. **PM6-3 delivered** 2026-09-25 as three commits, each under the review budget (`21c27a4d` 81, `cbde6fa9` 376, `901e77c1` 164; 621 in total): the `worktree-binding` record kind, the locked read/bind operations with the five-fact idempotence rule that excludes `created_at`, and the audit-driven proofs (corruption classification, sorted listing, whole-sandbox refusal inventories, lock release on refusal and on write failure, digest containment). **PM6-4 delivered** 2026-09-25 as one commit (`9c49f041`, 446 lines, 46 over the budget with the overage recorded): the `worktree inspect|provision|list` command surface, with the plan printed before confirmation, provisioning from exactly the displayed plan, registration through a narrow port that needed no growth of the command context, an unverifiable result binding nothing, and a registry failure staying a warning. **PM6-5 delivered**: the unit documentation, the acceptance record and the roadmap are updated, and the one product change it produced is the corrective display fix `64b075cd` on PM6-4's two paths — the confirmed plan now prints the exact command and the worktree's dirtiness, which the frozen design requires the human to see before confirming. PM-6 is delivered as a unit, and its deferred review pass closed on 2026-09-25 with 8 of 8 candidates approved and their authority burned.
...
- [x] **PM-7 — Implement the explicit Open Pi flow**
  ...
  - Planned 2026-09-25 in `odd/tasks/pm-7-open-pi-flow.md` after a read-only surface map, with the user's six decisions: **`tmux`-only in v1** (terminal emulators are an explicit non-goal, deferred until desktop behaviour can be verified for real), **`gentle-shell` launched in the capability worktree** (so the child can claim, heartbeat and bind itself), the **handoff passed by argv/initial prompt** (no file in the worktree, no new store record kind), the **background-subagent fallback offered as a visible confirmation**, the **session identity passed by the parent** rather than derived from the worktree path, and the action **triggered from the command surface** with the `[Open Pi]` affordance **absent rather than disabled**. The unit is four slices (PM7-1 readiness predicate and affordance, PM7-2 launch plan and `tmux` adapter, PM7-3 visible fallback, PM7-4 honest launch uncertainty plus documentation and verification); the four slices are delivered below.
  - **PM7-1 delivered** (`e0c1089f`, 274 diff lines) and **PM7-2 delivered** (`fc13aaad` + `d53d34ab`, 207 + 257): the readiness predicate with one named diagnostic per disqualifier, the injectable `tmux -V` host probe, the `[Open Pi]` affordance that is absent rather than disabled, the array-argv launch plan with the pinned structured handoff, the named detached tmux session with collision refusal, the `open` sub-action with a single confirmation, and the `session_start` receiver that writes the child's own binding and heartbeat. **PM7-3 delivered** (`f14a9dc4` + `79f72ea8` + `a5cb84c4`): the visible background-subagent fallback, offered only when the sole unmet gate is the host, launched through `childArguments` with the rpc mode pair replaced by `--print` (an undriven rpc child exits without working, measured against the binary), awaiting the child's `spawn`/`error` settlement before reporting, with the parent's interactive-host signal stripped. **PM7-4 delivered** (`79fdcb9e` plus the documentation closure): the two honest launch states, where `confirmed` is earned only by the child's own canonical `session-binding` for exactly this worktree, written after this launch, with a fresh heartbeat, and every other record is rejected (corruption is named; a record that is merely not evidence is skipped); the observation runs detached from the command so the request is reported first; `docs/project-map.md`, `docs/gentle-shell.md` and `README.md` document the flow; and the unit's nine acceptance criteria are traced to their tests, with criteria 7 and 9 marked **not verified** end to end because no real child has written its binding in the suite. **Corrective round after the acceptance-trace verification:** the trace and the reference were corrected in place, and the round found a real production bug — `tmux` does not forward a new client variable to the session it creates, so the interactive child never received its launch identity and wrote no binding, which made criterion 9 false in production while the suite's tmux test (a shell, not Pi) stayed green. The identity now travels as `tmux new-session -e`, pinned by a skip-guarded integration test against a real tmux server; the confirmation compares instants instead of strings, requires the heartbeat pid to match the binding, and takes the launched child's pid where the host reports one. Both paths were observed end to end once with real children. Recorded debt: a launch nonce for exact identity on the tmux path needs a store-schema field, so it is PM-8's decision; `worktree-path-escapes` has no readiness fixture because the derived path cannot reach it; and the worktree comparison is lexical, so a symlinked alias stays unconfirmed rather than wrongly confirmed. **Not `AgentRunner`:** the fallback is a detached one-shot launch with no child markers, no IPC channel and no lifecycle supervision, deliberately, because a runner-owned marker without a runner would misdescribe the process.
...
- 2026-09-24: PM-3 closed in `odd/tasks/pm-3-render-and-inspector.md` across five independently reviewed stages: PM3-1a/1b (card core, composition, rail slot, descriptor digest, narrow mode), PM3-2 (coverage explanations, grouping, collapse), PM3-3 (row selection, inspector, reveal), PM3-4 (interactive surface declaration, which closes PM-2's gap) and PM3-5 (documentation and verification of the unit). PM-4 through PM-9 remained unstarted at that point and each required independent authorization and task planning.
- 2026-09-26: The deferred native review passes are complete for PM-1 through PM-7. PM-5 closed 5 of 5 and PM-6 closed 8 of 8 on 2026-09-25; PM-7 closed 4 of 4 on 2026-09-26 with lineages `review-acf83d5f5f9353d6`, `review-ddc56437f14018d4`, `review-250a8c717f694672` and `review-34e872a914418e22`, all `approved` with their authority burned, no correction round, and nine non-blocking advisories recorded as debt. The reviewed tip is `6b9e87f1`, identical to the delivered unit, and RDD returned to `off` for this clone.

## Next decision

**PM-7 is complete as a unit, including its deferred native review pass.** PM-1 through PM-7 are delivered as units, and the deferred native review passes are complete for PM-1 through PM-7. PM7-1 through PM7-4 are delivered and measured (`e0c1089f` + `5cae623d`, `fc13aaad` + `d53d34ab`, `f14a9dc4` + `79f72ea8` + `a5cb84c4` + `45da3471`, `79fdcb9e` + the documentation closure), and on 2026-09-26 all four review candidates closed `approved` with their authority burned and nine non-blocking advisories recorded as debt.

The user's ratified order for what remains is: the **orchestrator session-tab layer** first, planned in `odd/tasks/orchestrator-session-tabs.md` — its four product decisions are already ratified, its row extends the existing sidebar header that PM-3 settled (so the collision with this initiative is zero), and it consumes only data PM-7 already produces, which is why the PM-7 handoff needed no new field for it — then **PM-8** (integration-readiness sequencing, which also owns the two schema-growth decisions this initiative recorded: per-capability verification requirements and a launch nonce for exact tmux identity), then **PM-9** (rollout and end-to-end verification). PM-8 and PM-9 remain unplanned. The tab layer is planned but not implemented: it carries one open shape decision before its first source write, because decision 2's list form (a capability repeated under each surface it requires) and the agreed single-row placement do not fit together at 140 columns without a truncation policy.
