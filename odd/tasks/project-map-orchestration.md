...
The user's ratified order for what remains is: the **orchestrator session-tab layer** first — its four product decisions are already ratified, its row enters the existing sidebar `vstack` that PM-3 settled (so the collision with this initiative is zero), and it consumes only data PM-7 already produces, which is why the PM-7 handoff needed no new field for it — then **PM-8** (integration-readiness sequencing, which also owns the two schema-growth decisions this initiative recorded: per-capability verification requirements and a launch nonce for exact tmux identity), then **PM-9** (rollout and end-to-end verification). PM-8 and PM-9 remain unplanned.
